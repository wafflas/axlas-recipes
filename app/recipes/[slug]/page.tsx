...
import { FaClock, FaHourglass, FaStar, FaUtensils } from "react-icons/fa";
...
export default async function RecipePage(context: {
  ...
  return (
    <main className="w-full min-h-screen bg-[var(--color-primary)]">
      {/* Hero */}
      {heroUrl && (
        <div className="relative max-w-5xl mx-auto px-4 md:px-6 pt-4">
      ...
      >
        <header className="pb-8">
          ...
          {data.summary && (
            <p className="pt-2 text-lg md:text-xl opacity-90">{data.summary}</p>
          )}

          {/* Meta */}
          <div className="pt-6 grid grid-cols-2 md:grid-cols-5 gap-4 text-lg text-black">
            <div>
              <strong className="inline-flex items-center gap-2">
                Prep
                <FaHourglass width="18" height="18" />
              </strong>
